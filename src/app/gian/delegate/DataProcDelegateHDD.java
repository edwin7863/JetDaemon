...
public class DataProcDelegateHDD
{
	...
	public void ProcessItemData(String tcompcode,String transDate,FileWriter fw) throws SQLException, IOException
	{
		...
		while(distItr.hasNext())
		{
			...
			line.append("0").append(",")                                                                     //A 
			    ...
			    .append("").append(",")                                                                      //F
			    .append("PCS").append(",")                                                                   //G
			    .append(model.get("prodname")==null?"":model.get("prodname").toString().toString().toString().replace(',', ' ').replace('\'', ' ').trim()).append(",")  //H
			    .append(model.get("origname")==null?"":model.get("origname").toString().toString().toString().replace(',', ' ').replace('\'', ' ').trim()).append(",")  //I
			    .append("").append(",")                                                                      //J
			    .append("").append(",")                                                                      //K
			    .append("USD").append(",")                                                                   //L
			    .append("0").append(",")                                                                     //M
	...
	//產生會計科目
	public void ProcessAccountData(String tcompcode,String transDate,String AccountType,FileWriter fw) throws SQLException, IOException
	{
		...
		while(distItr.hasNext())
		{
			...
			if(tcompcode.equals("H")||tcompcode.equals("I")||tcompcode.equals("M"))
			{
				CurrCheck="0";
				CurrID="";
			}
	...
	public void ProcessCustData(String tcompcode,String transDate,FileWriter fw) throws SQLException, IOException
	{
		...
		while(distItr.hasNext())
		{
			...
			line.append("0").append(",")                                                                  //A Serial
				...
				.append(custxxid).append(",")                                                             //C FundsAttribution
				.append(model.get("custname")==null?"":model.get("custname").toString().replace(',', ' ').replace('\'', ' ').trim()).append(",")      //D FullName
				.append(model.get("custname")==null?"":model.get("custname").toString().replace(',', ' ').replace('\'', ' ').trim()).append(",")      //E ShortName
				.append(model.get("datatype")==null?" ":model.get("datatype").toString().trim()).append(",") //class id                                                       //F ClassID
				.append("").append(",") //area id                                                        //G 
				.append(model.get("currency")==null?"":CommonUtil.getCurrency(tcompcode,model.get("currency").toString())).append(",")//H CurrencyID
				.append("FALSE").append(",")                                                              //I IsTemp
				.append(isFore).append(",")                                                               //J IsForeign
				.append(model.get("serialno")==null?" ":model.get("serialno").toString().trim()).append(",")      //K TaxNo
	...
	public void ProcessVdrData(String tcompcode,String transDate,FileWriter fw) throws SQLException, IOException
	{
	...
	public void ProcessReceviveData(String tcompcode,String transDate,FileWriter fw) throws SQLException, IOException
	{
		...
		dbUtil = new DbUtil(DB_TYPE, DB_NAME);
		db     = (MisDataBaseImp) dbUtil.getDataBase();
		List distList =  new ReceivexmfDao(db).findH(tcompcode,transDate);
		//if(distList.size()==0)
		//	return;
		...
		while(distItr.hasNext())
		{
			...
			String invostat = model.get("invostat")==null?"":model.get("invostat").toString().trim();//放付款日
			...
			line.append("0").append(",")                                                                   //A
			    ...
			    .append(origNewTtl).append(",")       //K Total
			    .append(taxAmt).append(",")                                                                //L tax
			    .append(Math.round(localNewTtl.doubleValue())).append(",")                                                           //M LocalTotal(有含推貿費的總額)
			    .append(Math.round(localTax)).append(",")                                                              //N LocalTAX
			    .append("0").append(",")                                                                   //O
			...
			while(lineItr.hasNext())
			{
				...
				newprice=b.setScale(3,   BigDecimal.ROUND_HALF_UP).doubleValue();//取第三位updated by Ashley 2012/11/30
	...
	//轉出進貨發票資訊
	public void ProcessInvoiceDataIn(String tcompcode,String transDate,FileWriter fw) throws SQLException, IOException
	{
	...
	//轉出銷貨發票資訊
	public void ProcessInvoiceDataOut(String tcompcode,String transDate,FileWriter fw) throws SQLException, IOException
	{
	...
	public void ProcessSalesData(String tcompcode,String transDate,FileWriter fw) throws SQLException, IOException
	{
		...
		dbUtil = new DbUtil(DB_TYPE, DB_NAME);
		db     = (MisDataBaseImp) dbUtil.getDataBase();
		List distList =  new ShipmatlmfDao(db).find1015TypeH(tcompcode,transDate);
		//if(distList.size()==0)
		//	return;
		CostxdaibfDao costDao = new CostxdaibfDao(db);
		//String today = DateTimeUtil.DateTimeFormatToString(new Date(), "MMddyyyy");
		...
		while(distItr.hasNext())
		{
			...
			line.append("0").append(",")                                                                  //1-1
			    ...
			    //.append("0").append(",")                                                                  //15漏掉LocalOffSet
	...
	public void ProcessRecvDiscount(String tcompcode,String transDate,FileWriter fw) throws SQLException, IOException
	{
		...
		while(distItr.hasNext())
		{
			...
			//double origTtl = taxAmt + origAmt;
			String taxcode=model.get("taxtype")==null?"":model.get("taxtype").toString();
			//計算本單總價及稅金
	...
	public void ProcessSaleDiscount(String tcompcode,String transDate,FileWriter fw) throws SQLException, IOException
	{
    	...
    	fw.write("-1,DistNO,FromNO,DistBalance,DispBillDate,Status,BillDate,SalesId,CustFlag,CustomerID,DueTo,AccMonth,EncashDay,TaxType,CurrID,ExchRate,Total,Tax,Discount,LocalTotal,LocalTax,DepartID,ProjectID,CashPay,SysID,VisaPay,CashPayStyle,VisaPayStyle,OtherPayStyle,OtherPay,LocOtherPay,InvoStyle,OffSet,VoucherNO,MakerID,Maker,PermitterID,Permitter,UDef1,UDef2,Remark,LocalOffSet,CostFlag,HasCheck,NoCheckOffSet,NoCheckDisCount,NoChkLocalOffSet,InvoFlag,InvoBillNO,PriceOfTax,MergeOutStat\n");
    	fw.write("-1,BillNO,RowNO,SerNO,ProdID,WareID,Quantity,MLPrice,TaxRate,TaxAmt,MLAmount,MLDist,BillDate,FromRow,FromNO,NeedUpdate,ProdName,HaveBatch,Price,Amount,HasCheck,SUnitID,SPrice,SQuantity,UnitRelation,EQuantity,EUnitID,EUnitRelation,CostForAcc,StdCost,ItemRemark,Dist,DistTaxAmt\n");
    	fw.write("-1,BillNO,BatchID,RowNO,SerNO,SQuantity,Quantity,ProduceDate,ValidDate,ParentRowNO,QtyRemain,FromNO,FromRow,TranType,BatchDist,MLDist,HasCheck,ItemRemark,NeedUpdate,EQuantity\n");
		Iterator distItr = distList.iterator();
		while(distItr.hasNext())
	 ...
	 * @throws IOException
	 */
	public void ProcessSaleReturn(String tcompcode,String transDate,FileWriter fw) throws SQLException, IOException
	{
		...
		dbUtil = new DbUtil(DB_TYPE, DB_NAME);
		db     = (MisDataBaseImp) dbUtil.getDataBase();
		List distList = new InteroutmfDao(db).findSaleReturnH("00",tcompcode,transDate);
		//if(distList.size()==0)
		//	return;
		...
		while(distItr.hasNext())
		{
			...
			//String taxcode="0";
			String taxcode=model.get("taxxcode")==null?"":model.get("taxxcode").toString();
			...
			//銷退本金(本幣)
			double localAmt = origAmt * exchrate;
			//銷退稅金(本幣)
			double localTax = origtaxAmt * exchrate;
			//銷退總金額(本幣)
			...
			line.append("0").append(",")                                                                  // -1
			    ...
			    .append("2").append(",")                                                                  //InvoStyle  2為發票空白
			    ...
			    .append(model.get("addrseqt")==null?"":model.get("addrseqt").toString()).append(",")      //AddrID 送貨地址
			...
			while(lineItr.hasNext())
			{
				...
				lineLine.append("1").append(",")                                                                       // -1
				        ...
				        .append(model.get("recvxloc")==null?"":model.get("recvxloc").toString()).append(",")          // WareID抓入庫倉庫
				        ...
				        .append(shipnubr).append(",")     //FromNO
				        .append(modelLine.get("seqtnubr")==null?"":modelLine.get("seqtnubr").toString()).append(",")  //FromRow
	 ...
	 * @throws IOException
	 */
	public void ProcessRecvReturn(String tcompcode,String transDate,FileWriter fw) throws SQLException, IOException
	{
		...
		dbUtil = new DbUtil(DB_TYPE, DB_NAME);
		db     = (MisDataBaseImp) dbUtil.getDataBase();
		List distList = new InteroutmfDao(db).findRecvReturnH("18",tcompcode,transDate);
		//if(distList.size()==0)
		//	return;
		//List distList =  new ShipmatlmfDao(db).findByType("10");
		CostxdaibfDao costDao = new CostxdaibfDao(db);
		...
		while(distItr.hasNext())
		{
			...
			while(lineItr.hasNext())
			{
				...
				lineLine.append("1").append(",")                                                                       // -1
				        ...
				        .append(recvnubr).append(",")     //FromNO
				        .append(modelLine.get("recordno")==null?"":modelLine.get("recordno").toString()).append(",")  //FromRow
				        .append(taxRate).append(",")      //TaxRate 
				        .append(lineTaxAmt).append(",")                                                                 //TaxAmt
				        .append("0").append(",")                                                                        //MLDist
				        .append("True").append(",")                                                                     //NeedUpdate
	...
	public void ProcessAdjustInventry(String tcompcode,String transDate,FileWriter fw) throws SQLException, IOException
	{
		...
		List distList = adjDAO.findListMfH(tcompcode,transDate);
		//if(distList.size()==0)
		//	return;
		CostxdaibfDao costDao = new CostxdaibfDao(db);
		//String today = DateTimeUtil.DateTimeFormatToString(new Date(), "MMddyyyy");
	 ...
	 * @throws IOException
	 */
	public void ProcessBorrowIn(String tcompcode,String transDate,FileWriter fw) throws SQLException, IOException
	{
		...
		List distList = proDAO.findListMfH(tcompcode,transDate);
	 ...
	 * @throws IOException
	 */
	public void ProcessRetBorrow(String tcompcode,String transDate,FileWriter fw) throws SQLException, IOException
	{
		...
		dbUtil = new DbUtil(DB_TYPE, DB_NAME);
		db     = (MisDataBaseImp) dbUtil.getDataBase();
		List distList = new InteroutmfDao(db).findBorrowReturnH("19",tcompcode,transDate);
	 ...
	 * @throws IOException
	 */
	public void ProcessFundsBill(String tcompcode,String transDate,FileWriter fw) throws SQLException, IOException
	{
		System.out.println("=====START TO PROCESS Funds Bill DATA======");
		DbUtil dbUtil     = null;	
		MisDataBaseImp db = null;
		dbUtil = new DbUtil(DB_TYPE, DB_NAME);
		db     = (MisDataBaseImp) dbUtil.getDataBase();
		 ...
		 * 1-fundbilldf(沖款明細)
		 * 2-fundbilldf(自訂收入)
		 * 3-fundprepayused(載入預收)
		 * 7-fundprepayused(載入預收)
		 * 4-fundaddprepay(新增預收)--對應
		 * 
		 */
		...
		line.append("0").append(",")                                                              //-1               
		    ...
		    .append(model.get("currency")==null?"":CommonUtil.getCurrency(tcompcode,model.get("currency").toString())).append(",")//CurrID           
		    .append(exchrate).append(",")//ExchRate         
		    .append("").append(",")                                                              //VoucherNo        
		    .append(model.get("cvname")==null?"1":model.get("cvname").toString()).append(",")    //VoucherAbs       
		    .append("0").append(",")                                                             //Cash             
		    .append("0").append(",")                                                             //Visa             
		    .append("0").append(",")                                                             //BillMoney        
		    ...
		    .append(model.get("sumpayment")==null?"0":model.get("sumpayment").toString()).append(",")//SumOffset        
		    .append(model.get("prepayused")==null?"0":model.get("prepayused").toString()).append(",")//PrepayUsed          
		    .append(model.get("selfmoney")==null?"0":model.get("selfmoney").toString()).append(",")//OrigSelfMoney       
		    .append(model.get("origprepayused")==null?"0":model.get("origprepayused").toString()).append(",")//OrigAddPrepay///TODO:       
		    .append(model.get("origprepayused")==null?"0":model.get("origprepayused").toString()).append(",")//OrigPrepayUsed///TODO:
		    .append(model.get("paytype1")==null?"0":model.get("paytype1").toString()).append(",")//CashStyleID         
		    .append(model.get("paytype2")==null?"0":model.get("paytype2").toString()).append(",")//VisaStyleID         
		    .append(model.get("paytype3")==null?"0":model.get("paytype3").toString()).append(",")//OtherPayStyleID     
		    .append(model.get("sumpayment")==null?"0":model.get("sumpayment").toString()).append(",") //OtherPay            
		    .append("True").append(",")	                                                          //IsAllCurrID         
		    .append("").append(",")                                                            //PermitterID         
		    .append(model.get("makerid")==null?"1":model.get("makerid").toString()).append(",") //MakerID             
		    ...
		    .append("False").append(",")                                              //UsePrepay 是否用預付要改///TODO:         
			...
			if(billtype==3)
			{
				...
				{					
					BasicDynaBean modelLine = (BasicDynaBean) lineItr.next();
					//OffSetMoneyt沖款金額計算
					String offsetmoney="0";
					String currency=model.get("currency")==null?"":model.get("currency").toString();//抓轉單的弊別
					if(currency.equals("N"))
					{
						offsetmoney=modelLine.get("localpayment")==null?"":modelLine.get("localpayment").toString();
					}
					else
					{
						offsetmoney=modelLine.get("payment")==null?"":modelLine.get("payment").toString();
					}
					StringBuffer lineLine = new StringBuffer();
					lineLine.append("1").append(",")                                                                    //-1             
				...
				while(lineItr2.hasNext())
				{
			        ...
			        .append(modelLine.get("origtrannubr")==null?"":modelLine.get("sysid").toString()).append(",")     //OriginNo         
			        .append(modelLine.get("fromordrnubr")==null?"":modelLine.get("sysid").toString()).append(",")     //FromOrderNo     
			        ...
			        .append(modelLine.get("trandate")==null?"":modelLine.get("trandate").toString()).append(",") //FundBillDate         
			        .append(modelLine.get("fromserial")==null?"":modelLine.get("fromserial").toString()).append(",")//FromRowNo    
			        .append(modelLine.get("fromdescription")==null?"":modelLine.get("fromdescription").toString()).append(",")  //FromDescription  
			        .append("A").append(",")  //DepartID        
			        .append(modelLine.get("localusedamount")==null?"":modelLine.get("localusedamount").toString()).append(",") //LocalUseMoney                                            
			        .append("\n")
			        ;
					fw.write(lineLine.toString());
				}
			...
			{
				lineList =  new FundbillDao(db).findAddPrepay(model.get("trannubr")==null?"":model.get("trannubr").toString());
				Iterator lineItr = lineList.iterator();
				while(lineItr.hasNext())
				{				
					BasicDynaBean modelLine = (BasicDynaBean) lineItr.next();
			        ...
			        .append("").append(",")//NoChkAddOffSet  
			        .append(modelLine.get("description")==null?"":modelLine.get("description").toString()).append(",") //Description    
			        .append(modelLine.get("currency")==null?"":CommonUtil.getCurrency(tcompcode,modelLine.get("currency").toString())).append(",") //CurrID     
			        .append(modelLine.get("cvid")==null?"":modelLine.get("cvid").toString()).append(",") //CustomerID         
			        .append(modelLine.get("trandate")==null?"":modelLine.get("trandate").toString()).append(",")//FundBillDate    
			        ...
			        .append("").append(",") //NoChkLocAddOffset                                            
			        .append("\n")
			        ;
				}
			}
			...
			{
				lineList =  new FundbillDao(db).findFundbillDf(model.get("trannubr")==null?"":model.get("trannubr").toString());
				lineList2 =  new FundbillDao(db).findSelfset(model.get("trannubr")==null?"":model.get("trannubr").toString());
				Iterator lineItr = lineList.iterator();
				Iterator lineItr2 = lineList2.iterator();
				int rowno = 1,rowno2=1;
				while(lineItr.hasNext())
				{					
					BasicDynaBean modelLine = (BasicDynaBean) lineItr.next();
					//OffSetMoneyt沖款金額計算
				...
				{					
			        ...
			        .append(modelLine.get("trannubr")==null?"":modelLine.get("trannubr").toString()).append(",")//FundBillNo     
			        .append(modelLine.get("serial")==null?"":modelLine.get("serial").toString()).append(",")    //SerNo          
			        .append(modelLine.get("isloan")==null?"":modelLine.get("isloan").toString()).append(",")   //IsLoan          
			        ...
			        .append(modelLine.get("memo")==null?"":modelLine.get("memo").toString()).append(",")//Memo   
			        .append("A").append(",")                                                               //DepartID    
			        .append(rowno2).append(",")                                                                //RowNo                                                     
			        .append("\n")                   
			        ;
					fw.write(lineLine.toString());
					rowno2++;
